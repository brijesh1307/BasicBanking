...
public class TransactionHistoryAdapter extends RecyclerView.Adapter<TransactionHistoryAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, int position) {
        ...
        viewHolder.fromName.setText(transactionArrayList.get(position).getFromName());
        viewHolder.toName.setText(transactionArrayList.get(position).getToName());
        viewHolder.amountTransferred.setText(String.format("%d", transactionArrayList.get(position).getAmountTransferred()));


        if (transactionArrayList.get(position).getStatus() == String.valueOf(1) ) {
